...
export class MakeRoomPage {
  ...
  constructor(public alertCtrl: AlertController, public navParams: NavParams,
    ...
    this.user_uid = navParams.data.user_id;
    this.spotList = ["한동대학교", "포항역", "양덕", "고속버스터미널", "시외버스터미널", "북부해수욕장", "육거리", "직접입력"];
    af.list('/userProfile/' + navParams.data.user_id).subscribe(data => {this.user_id = data[2].$value; this.user_name = data[3].$value});
  }
    ...
    if (this.bookingDate == this.nowDate && this.nowTime > this.bookingTime)
    {
      ...
      alert.addButton({
        ...
        handler: data => {
          ...
          participants_list.push({"uid": this.user_uid, "name": this.user_name});
          let url;
          ...
          if((this.nowDate+this.nowTime)>(this.bookingDate+this.bookingTime)){
            ...
            this.navCtrl.setRoot(ChatRoomPage, {chat_room_id: url.key, bookingDate:this.bookingDate, user_id: this.user_uid, whichPage: "makeRoom"});
          }
        }
      });
    }
    ...
    let alert = this.alertCtrl.create({
      ...
      alert.addButton({
        ...
        handler: data => {
          ...
          this.navCtrl.setRoot(ChatRoomPage, {chat_room_id: url.key, bookingDate:this.bookingDate, user_id: this.user_uid});
        }
      });
      alert.setTitle('탑승인원');
      alert.present();
